...
mandatory_keys = 'key.txt'
mandatory_key_values = 'value.txt'
'''
...
def lambda_handler(event, context):
    ...
    ids = []
    #autoscaling_res = boto3.resource('autoscaling')
    ...
    try:
        ...
        if eventname == 'CreateAutoScalingGroup':
            ...
            ids.append(detail['requestParameters']['autoScalingGroupName'])
            logger.info(ids)
        else:
        ...
        if ids:
            ...
            for resourceid in ids:
                try:
                    ...
                    princip = principal
                res_list = [resourceid]
                global resourceid_global
                resourceid_global=resourceid
                ...
                default_tags = [{'Key': 'Owner', 'Value': user,'ResourceId': resourceid, 'ResourceType': 'auto-scaling-group', 'PropagateAtLaunch':False},
                                            {'Key': 'PrincipalId', 'Value': principal, 'ResourceId': resourceid, 'ResourceType': 'auto-scaling-group', 'PropagateAtLaunch':False},
                ...
                if len(all_key_names) > 0:
                    ...
                    for key in all_key_names:
                        default_tags.append({'Key': key, 'Value': all_default_values[c], 'ResourceId': resourceid,'ResourceType': 'auto-scaling-group','PropagateAtLaunch': False})
                        c = c + 1
                ...
                if eventname == 'CreateAutoScalingGroup':
                    print('Attempting to bind tags to ASG: ', resourceid)
                    snap_filter = [{'Name': 'autoScalingGroupName', 'Values': [resourceid]}]
...
def tag_gen(tags, res_list, default_tags):
    ...
    for tag in tags:
        ...
        try:
            ...
            if name.startswith('aws:'):
                ...
                new_tag = {'Key': tag['Key'], 'Value': tag['Value'], 'ResourceId': resourceid_global,'ResourceType': 'auto-scaling-group','PropagateAtLaunch': False}
    ...
    """
    logger.info('Default Tags' + str(default_tags))
    logger.info('new_tags_list Tags' + str(new_tags_list))

    final_tags = add_default_tags(default_tags, new_tags_list)
    logger.info('final_tags Tags' + str(final_tags))
    ...
    try:
        ...
        client.create_or_update_tags(Tags=final_tags)
...
def add_default_tags(default_tags, new_tags_list):
    ...
    try:
        for x in default_tags:
            ...
            if in_list == False:
                new_tag = {'Key': x['Key'], 'Value': x['Value'], 'ResourceId': resourceid_global,'ResourceType': 'auto-scaling-group','PropagateAtLaunch': False}
...
def get_bucket_name():
     ...
     #acc_sdlc = os.environ['account'] # need tobe modified
     #bucket_region = os.environ['bucket_region']
     bucket_region = 'us-west-2'
     #local_bucket_name = 'global-variables-' + acc_sdlc + '-' + bucket_region
     #bucket_name = local_bucket_name
     bucket_name = 'lamda-asg-autotagging'
     # ============================================
