...
def lambda_handler(event, context):
    ...
    try:
        ...
        if eventname == 'CreateDBInstance':
            ids.append(detail['responseElements']['dbiResourceId'])
            logger.info(ids)
        ...
        if ids:
            ...
            for resourceid in ids:
                ...
                if eventname == 'CreateDBInstance':
                    ...
                    dbi_filter = [{'Name': 'db-instance-id', 'Values': [resourceid]}]
                    for dbinstance in client.describe_db_instances(Filters = dbi_filter)['DBInstances']:
                        if 'Tags' in dbinstance:
                            ...
                            RDS_client.add_tags_to_resource(Resources=res_list, Tags=default_tags)
